...
//                        onMachineRunning(machineId);
                        ...
                        machineServiceClient.getMachine(event.getMachineId()).then(new Operation<MachineDto>() {
                            ...
                            public void apply(MachineDto arg) throws OperationException {
                                eventBus.fireEvent(new MachineStateEvent(arg, MachineStateEvent.MachineAction.RUNNING));
                            }
                        }).catchError(new Operation<PromiseError>() {
                            @Override
                            public void apply(PromiseError arg) throws OperationException {
                                Log.error(getClass(), "Machine fetching error. " + arg.getMessage());
                            }
                        });
                        break;
                    case ERROR:
                        Log.error(getClass(), "Machine creation error. " + event.getError());
